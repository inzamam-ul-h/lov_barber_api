...
if (! function_exists('ecom_get_coupons_array'))
{
    function ecom_get_coupons_array($record)
    {
        $image = $record->image;
        ...
        $array = array();
        $array['id'] = $record->id;
        $array['coupon_code'] = $record->coupon_code;
        ...
        $array['max_discount_value'] = $record->max_discount_value;
...

	{
		...
		if($record->module == 'sod'){
			...
			if($record->type == 'order'){
				...
				$addon = DB::table('order_addons')->where('order_id', $record->type_id)->first();
				...
				if(!empty($addon))
				{
					$addon_order_id = $addon->order_id;
					$addon_total_value = $addon->total_value;
					$addon_vat_value = $addon->vat_value;
					$addon_final_value = $addon->final_value;
					$addon_details = DB::table('order_addons_details')->where('addon_id', $addon->id)->get();
				}
				...
				if(!empty($addon) )
				{
					$new_arr['order_id'] = $addon_order_id;

					$new_arr['addon_details'] = $addon_details;

					$new_arr['total_value'] = $addon_total_value;

					$new_arr['vat_value'] = $addon_vat_value;

					$new_arr['final_value'] = $addon_final_value;

					$array['addons'] = $new_arr;
				}
