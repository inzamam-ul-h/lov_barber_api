...
class RewardController extends Controller
{
    ...
    public function admin_datatable(Request $request)
    {
        ...
        $Records = Reward::leftJoin('svc_vendors', 'rewards.vendor_id','=', 'svc_vendors.id')
            ->select(['rewards.id','rewards.silver_punches','rewards.golden_punches','rewards.platinum_punches','rewards.min_order_value','rewards.type', 'rewards.status', 'rewards.vendor_id', 'svc_vendors.name as vendor_name']);
        ...
        $response= Datatables::of($Records)
            ->filter(function ($query) use ($request) {
                ...
                {
                    $query->where('rewards.vendor_id', '=', "{$request->get('vendor_id')}");
                }
               
                if ($request->has('type') && $request->get('type') != -1)
                {
                    $query->where('rewards.type', '=', "{$request->get('type')}");
                }
            ...
            ->addColumn('min_no_of_orders_to_claim_silver_house', function ($Records) {
                $record_id = $Records->id;
                $title = $Records->min_no_of_orders_to_claim_silver_house;
    ...
    public function show($id)
    {
        ...
        if($Auth_User->can($this->view_permission) || $Auth_User->can('all'))
        {
           ...
           // $rest_id = $Model_Data->rest_id;
            ...
            $reward_discount_types = RewardDiscountType::select(['name'])->where('id',$Model_Data->discount_type)->orderby('name', 'asc')->get();

            $discount_type="";
            foreach($reward_discount_types as $reward_discount_type)
            {
                $discount_type = $reward_discount_type->name;
            }
            ...
            return view($this->views_path.'.show',compact('Model_Data', 'vendors_array', 'typ', 'discount_type'));
        }
        else
        {
...
//             }
    ...
    public function makeActive($id)
    {
        ...
        if($Auth_User->can($this->status_permission) || $Auth_User->can('all'))
        {
            ...
            // $rewards_data = Reward::where('rest_id',$rest_id)->where('status',1)->get();
