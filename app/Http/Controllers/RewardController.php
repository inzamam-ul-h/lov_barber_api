...
class RewardController extends Controller
{
    ...
    public function admin_datatable(Request $request)
    {
        ...
        $Records = Reward::select([
            'rewards.id',
            'rewards.silver_punches',
            'rewards.golden_punches',
            'rewards.platinum_punches',
            'rewards.min_order_value',
            'rewards.status',
        ]);
        ...
        $response = Datatables::of($Records)
            ...
            ->addColumn('action', function ($Records) {
                ...
                if ($Auth_User->can($this->view_permission) || $Auth_User->can('all')) {
                    $str .= ' <a class="btn btn-outline-primary" href="' . url('reward/' . $record_id) . '" title="View Details">
				<i class="fa fa-eye"></i>
				</a>';
                }
                ...
                if ($Auth_User->can($this->edit_permission) || $Auth_User->can('all')) {
                    if ($status == 0) {
                        $str .= ' <a class="btn btn-outline-primary" href="' . url('reward/' . $record_id . '/edit') . '" title="Edit Details" >
    ...
    public function makeActive($id)
    {
        ...
        if ($Auth_User->can($this->status_permission) || $Auth_User->can('all')) {
            ...
            $rewards_data = Reward::where('status', 1)->first();
